...
async def list_orders(
    ...
    service = OrderService(db)
    result = await service.list_orders(current_user.id, page=page, page_size=page_size)
...
@router.get(
    ...
    description=(
        "Returns carrier name, tracking number and mock event timeline.  \n"
        "Real carrier integration is `BACKEND DECISION REQUIRED`."
    ),
)
...
@router.get(
    ...
    summary="Admin — get order invoice stub",
    description="Authorization: `orders.view`. Returns invoice number and issued date.",
)
async def admin_get_invoice(
    ...
    return InvoiceResponse(order_id=data["order_id"], **{
        k: v for k, v in data.items() if k != "order_id"
    })
